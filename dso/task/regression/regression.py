...
class RegressionTask(HierarchicalTask):
    ...
    def reward_function(self, p, optimizing=False):
        ...
        spearman_corr1, _ = spearmanr(y_hat[:4], self.y_train[:4])
        spearman_corr2, _ = spearmanr(y_hat[4:8], self.y_train[4:8])
        spearman_corr3, _ = spearmanr(y_hat[8:12], self.y_train[8:12])
        spearman_corr4, _ = spearmanr(y_hat[12:], self.y_train[12:])

        r = (spearman_corr1+spearman_corr2+spearman_corr3+spearman_corr4) / 4
        if r < 0 or r!=r:
            r = 0
        ...
        if r == 1:
            ...
            fig, ax = plt.subplots()
            ax.text(0.5, 0.5, data, fontsize=12, ha='center')

            # 隐藏坐标轴
            ax.axis('off')

            # 保存图像
            fig.savefig('dso/output/'+ str(p) + '.png')
            # with open('dso/output/'+ str(p) + '.txt', 'w', encoding='utf-8') as file:
            #     file.write(data)

            filename = 'dso/output/'+ str(p) + '.csv'
            with open(filename, mode='w', newline='') as file:
                writer = csv.writer(file)
                
                # 对于单列数据，每行只写入一个数据项
                for i in range(len(y_hat)):
                    writer.writerow([y_hat[i]])  # 注意：writerow() 方法接收的是一个列表
            
        # Compute metric
        # r = self.metric(y_hat, self.y_train)
...
def make_regression_metric(name, y_train, *args):
    ...
    all_metrics = {
        ...
        # "inv_nrmse_pde" :    (lambda y, y_hat : 1/(1 + args[0]*np.sqrt(np.mean((y - y_hat)**2)/var_y)),
        #                 1),
